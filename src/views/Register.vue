...
<div>
    <div id="register" class="myCard">
        <div @keyup.prevent.enter="register" ref="form" lazy-validation class="userBox">
            ...
            <v-text-field v-if="$store.state.errors.userName" v-model="userName" label="User Name" color="#a04b4b" required :rules="[() => $store.state.errors.userName]"
                error></v-text-field>
            ...
            <v-text-field v-else v-model="password" color="secondary" label="Password" required :type="show1 ? 'text' : 'password'"
                ...
                <v-text-field v-if="$store.state.errors.password2" v-model="password2" label="Confirm Password" required :rules="[() => $store.state.errors.password2]"
                error :type="show2 ? 'text' : 'password'" :append-icon="show2 ? 'visibility' : 'visibility_off'"
